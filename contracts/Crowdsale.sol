...
contract Crowdsale is Pausable {
   ...
   uint256 public limitTier4 = 50e24;//chyba: unused
   ...
   function setTierRates(uint256 tier1, uint256 tier2, uint256 tier3, uint256 tier4)//chyba: melo by byt nastavitelne pouze jednou, idealne pred zacatkem
   ...
   function checkCompletedCrowdsale() public whenNotPaused {//chyba: pausing is problematic due to trust, should be time limited
      if(!isEnded) {
         if(hasEnded() && !goalReached()){
            vault.enableRefunds();

            isRefunding = true;
            isEnded = true;//chyba: could be higher
            Finalized();
   ...
   function claimRefund() public whenNotPaused {
     ...
     token.refundTokens(msg.sender, tokensBought[msg.sender]);//chyba: is this necessary?, maybe make tokens not transferrable during sale
   }
   ...
   function calculateExcessTokens(
      ...
      if(tierSelected != 4)
         tokensNextTier = calculateTokensTier(weiNextTier, tierSelected.add(1));//chyba: might overstep
      else
         returnTokens = true;
      ...
      if(returnTokens) msg.sender.transfer(weiNextTier);//chyba: never used & constant function can't contain transfer, plus misuderstanding conding practices, plus can this ever happen?
   }
   ...
   function validPurchase() internal constant returns(bool) {
      ...
      bool hasBalanceAvailable = crowdsaleBalances[msg.sender] < maxPurchase;//chyba: maxPurchase utility dubious, can be multiple transactions from different addresses, consider adding mxgas check as a measure against whales
